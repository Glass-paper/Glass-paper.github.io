...
contract ProductManagement {
    ...
    */
    ...
    function buildProcess(address manuf,string memory serial_number,string memory manufacturerName,string memory longLat, string memory process_type, string memory creation_date) public returns (bytes32){
       ...
       // require(processes[process_hash].manufacturer == address(0), "Process ID already used");
        ...
        Process memory new_process = Process(manuf, serial_number,manufacturerName,longLat, process_type, creation_date);
        processes[process_hash] = new_process;
        emit processhash(process_hash);
        return process_hash;
    }

    function buildProduct(string memory serial_number,string memory manufacturerName,string memory longLat, string memory product_type, string memory creation_date, bytes32[] memory process_array) public returns (bytes32){
        ...
        Product memory new_product = Product(msg.sender, serial_number,manufacturerName,longLat, product_type, creation_date, process_array);
